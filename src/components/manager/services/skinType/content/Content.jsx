...
const Content = ({setItemUpdate, setShowModal}) => {
  const [skinTypeItemsActive, setSkinTypeItemsActive] = useLocalStorage(LOCALSTORAGE_NAME.SKIN_TYPE_ITEMS_ACTIVE, "")
  const [itemActive, setItemActive] = useState([]);

  const handleOnClick = (item) => {
    let list = []
    if (itemActive.includes(item.id)) {
        list = itemActive.filter((element) => Number(element) !== Number(item.id))
    } else {
